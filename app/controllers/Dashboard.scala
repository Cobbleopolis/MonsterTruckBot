...
class Dashboard @Inject()(implicit db: Database, webJarAssets: WebJarAssets, override val env: RuntimeEnvironment, ws: WSClient, messages: MessagesApi, discordBot: DiscordBot, configuration: Configuration) extends Controller with SecureSocial with I18nSupport {

    def dashboard(guildId: String) = SecuredAction(implicit request => {
    ...
    def submitCoreSettings(guildId: String): Action[AnyContent] = SecuredAction(implicit request => {
        implicit val userOpt: Option[User] = Some(request.user.asInstanceOf[User])
